...
export default function GovernmentAdminChangePasswordPage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      const response = await fetch('/api/government-admin/verify', { credentials: 'include' });
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await fetch('/api/government-admin/change-password', {
        method: 'POST',
        ...
        <Card className="w-full border-slate-200 bg-white">
          <CardHeader>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              You must replace the temporary password before accessing the dashboard.
            </p>
          </CardHeader>
          <CardContent>
