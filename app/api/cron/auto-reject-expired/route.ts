...
// Vercel Cron Job for Auto-rejecting Expired Service Offers
// This endpoint will be called automatically by Vercel Cron
export async function GET(request: NextRequest) {
  try {
    console.log('🔄 Starting automated auto-rejection process...');
    
    // Verify this is coming from Vercel Cron
    // Vercel automatically adds the authorization header for cron jobs
    const authHeader = request.headers.get('authorization');
    
    // Optional additional security check with custom secret
    ...
    // Vercel cron jobs also include a specific header
    ...
    console.log(`🎉 Auto-rejection process complete. Rejected ${rejectedOffers.length} offers`);
    ...
    console.error('❌ Auto-rejection cron job error:', error);
    return NextResponse.json({ 
      error: 'Auto-rejection cron job failed',
...
// Also support POST method for manual triggering (backward compatibility)
